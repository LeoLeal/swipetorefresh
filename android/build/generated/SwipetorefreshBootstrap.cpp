...
#define TAG "br.com.leoleal.swipetorefresh"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Swipetorefresh_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Swipetorefresh.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::SwipetorefreshBindings::lookupGeneratedInit(
...
static void Swipetorefresh_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Swipetorefresh_getBinding)->GetFunction());
}

static void Swipetorefresh_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::SwipetorefreshBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry SwipetorefreshBinding = {
	"br.com.leoleal.swipetorefresh",
	Swipetorefresh_init,
	Swipetorefresh_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_br_com_leoleal_swipetorefresh_SwipetorefreshBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("br.com.leoleal.swipetorefresh", &SwipetorefreshBinding);
	titanium::KrollBindings::addExternalLookup(&(::SwipetorefreshBindings::lookupGeneratedInit));
}
